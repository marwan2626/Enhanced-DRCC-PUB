...
def calculate_sensitivity(heatpumpForecast, opf_results, time_steps):
    ...
    for t in range(1, len(time_steps)):
        ...
        for line in line_currents_prev.keys():
            if line_currents_t[line] > 0.180:  # Only calculate if current > 66% of the line limit
                sensitivity_value = ((w_t) / (line_currents_t[line]))
            else:
                sensitivity_value = 0  # Set sensitivity to 0 if current <= 66% of the line limit
        ...
        time_list = [611, 612, 613, 614, 615, 617, 618, 619, 620]

        # if t in time_list:
        #     print(f"Time step {t}: Sensitivity calculated for line 24 = {sensitivity_results[t][24]}")
        #     print(f"Time step {t}: Line 24 current at previous timestep = {line_currents_prev[24]}")
        #     print(f"Time step {t}: Line 24 current at current timestep = {line_currents_t[24]}")
...
def calculate_omega_I(alpha, sensitivity, cov_matrix, Omega_I):
    ...
    for t, sensitivity_t in sensitivity.items():
        for line, sensitivity_value in sensitivity_t.items():
            ...
            Omega_I_new[t][line] = scaling_factor * np.linalg.norm(scaled_sensitivity)*1e-3  # to kA

            # Check if Omega_I_new is zero and it wasn't zero in Omega_I
            if Omega_I_new[t][line] == 0 and Omega_I[t][line] != 0:
                Omega_I_new[t][line] = Omega_I[t][line]  # Retain the old value to avoid stagnation at max_I 
            
            if Omega_I_new[t][line] >= 0.27:
                print(f'Omega_I line {line} at time step {t} = {Omega_I_new[t][line]}')
                print(f'sensitivity line {line} at time step {t} = {sensitivity_t[line]}')
        ...
        for i, bus in enumerate(net.bus.index):
            if bus in net.load.bus.values:
                if bus in flexible_load_buses:
                    ...
                    model.addConstr(
                        ts_out_vars[t][bus] <= ts_size_mwh_scaled_dict[bus]*1000,
                        name=f'storage_charging_{t}_{bus}'
                    )
...
def drcc_opf(net, time_steps, const_load_heatpump, const_load_household, Bbus, heatpumpForecast, heatpump_scaling_factors_df, max_iter_drcc, alpha, eta):
    ...
    initial_results = solve_opf(net, time_steps, const_load_heatpump, const_load_household, heatpump_scaling_factors_df, Bbus, Omega_I_init)    #calcualte sensitivity from forecast & real data
    
    sensitivity = calculate_sensitivity(heatpumpForecast, initial_results, time_steps)
    ...
    for drcc_iter in range(max_iter_drcc):
        ...
        #Apply threshold limit to Omega_I at first iteration
        for t in Omega_I:
            for line in Omega_I[t]:
                if Omega_I[t][line] >= 0.27:
                    Omega_I[t][line] = 0.27
                if line == 24:
                    print(f"Omega_I for line 24 at iteration {drcc_iter + 1} = {Omega_I[t][line]}")
        ...
        previous_results = drcc_opf_results
