...
def box_line_loading_two_subplots(mc_line_results_df1, mc_line_results_df2):
    ...
    axes[0].axhline(100, color="red", linestyle="--", linewidth=1.5, label="100% Loading")  # Add dashed line
    axes[0].set_ylim(0, 120)  # Set y-axis limits
    axes[0].set_ylabel("Loading Percentage (%)")
    axes[0].set_title("Line Loading Percentages Across for deterministic optimization")
    axes[0].tick_params(axis='x', rotation=45, labelsize=10)  # Enable x-axis ticks and rotate them
    axes[0].set_xlabel("Line Index")  # Add x-axis label
    axes[0].legend()  # Add legend for the dashed line
...
def box_line_loading_two_subplots_percentile(mc_line_results_df1, mc_line_results_df2):
    # Create a figure with two subplots
    fig, axes = plt.subplots(2, 1, figsize=(15, 10), sharex=False)

    def add_percentile_marker(data, ax, percentile=95, color="black"):
        """Add a marker for the given percentile to the box plot."""
        percentiles = data.groupby("line")["loading_percent"].quantile(percentile / 100.0)
        for i, (line, percentile_value) in enumerate(percentiles.items()):
            ax.scatter(
                i,  # Boxplot x-coordinate for the line
                percentile_value, 
                color=color,
                marker="x",  # Shape of the marker
                s=20,        # Size of the marker 
                zorder=3, 
                label=f"{percentile}th Percentile" if i == 0 else ""  # Avoid duplicate labels in legend
            )
    ...
    add_percentile_marker(mc_line_results_df1, axes[0], percentile=95, color="black")  # Add 95th percentile markers
    axes[0].axhline(100, color="red", linestyle="--", linewidth=1.5, label="100% Loading")  # Add dashed line
    axes[0].set_ylim(0, 140)  # Set y-axis limits
    axes[0].set_ylabel("Loading Percentage (%)")
    axes[0].set_title("Line Loading Percentages Across Monte Carlo Samples (OPF)")
    axes[0].tick_params(axis='x', rotation=45, labelsize=10)  # Enable x-axis ticks
    axes[0].set_xlabel("Line Index")  # Add x-axis label
    ...
    add_percentile_marker(mc_line_results_df2, axes[1], percentile=95, color="black")  # Add 95th percentile markers
    axes[1].axhline(100, color="red", linestyle="--", linewidth=1.5, label="100% Loading")  # Add dashed line
    axes[1].set_ylim(0, 140)  # Set y-axis limits
    axes[1].set_xlabel("Line Index")
    axes[1].set_ylabel("Loading Percentage (%)")
    axes[1].set_title("Line Loading Percentages Across Monte Carlo Samples (DRCC)")
    axes[1].tick_params(axis='x', rotation=45, labelsize=10)  # Rotate x-axis labels
